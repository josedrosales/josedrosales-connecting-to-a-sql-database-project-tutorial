...
try:
    with engine.connect() as connection:
        connection.execute(create_tables_statement)
...
try:
    with engine.connect() as connection:
        connection.execute(insert_data_statement)
        print("Data inserted into publishers, authors, books and book_authors.")
...
table_name_to_read = "authors"

try:
    df = pd.read_sql(f"SELECT * FROM {table_name_to_read}", con=engine)

    # Print the DataFrame
    print(f"Data from {table_name_to_read} table:")
    print(df)
